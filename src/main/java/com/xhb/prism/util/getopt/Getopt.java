...
package com.xhb.prism.util.getopt;
...
private ResourceBundle _messages = /*ResourceBundle.getBundle(
    ...
    new ListResourceBundle() {
        ...
        protected Object[][] getContents() {
            return new Object[][] {
                    {"getopt.ambigious", "{0}: ambigious {1}"}, 
                    {"getopt.arguments1", "{0}: arguments1 {1}"}, 
                    {"getopt.arguments2", "{0}: arguments2 {1}"}, 
                    {"getopt.requires", "{0}: requires {1}"}, 
                    {"getopt.unrecognized", "{0}: unrecognized arg {1}"}, 
                    {"getopt.unrecognized2", "{0}: unrecognized arg {1} {2}"}, 
                    {"getopt.illegal", "{0}: illegal arg {1}"}, 
                    {"getopt.invalid", "{0}: invalid arg {1}"}, 
                    {"getopt.requires2", "{0}: requires arg for {1}"}, 
            };
        }
    };
...
public List<String> getArgv() {
    return Arrays.asList(argv).subList(optind, argv.length);
}
...
public String getArg(int index) {
    ...
    if (index < argv.length)
        return argv[index];
    index -= optind;
    throw new IllegalArgumentException("too less argments, require " + index + 1);
}

public String shiftArg() {
    if (optind < argv.length)
        return argv[optind++];
    throw new IllegalArgumentException("too less argments");
}

public void endArgv() {
    if (optind < argv.length)
        throw new IllegalArgumentException("too more argments");
}
