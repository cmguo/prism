...
public class Collections {
     ...
     */
    public static <E> E accumulate(final Collection<E> elements, E init, BinaryOperation<E> op) {
    ...
    public static <E extends Number> E addup(Collection<E> numbers, Class<E> clazz) {
        if (null == clazz) {
    ...
    public static <E, T> Collection<T> map(final Collection<E> c, final Mapper<E, T> m) {
        return new AbstractCollection<T>() {
            @Override
            public Iterator<T> iterator() {
                return new MapFilter<E, T>(c.iterator(), m);
            }
            @Override
            public int size() {
                return c.size();
    ...
    private static class MapFilter<E, T> implements Iterator<T> {
        private Iterator<E> mIter;
        private Mapper<E, T> mMapper;
        MapFilter(Iterator<E> iter, Mapper<E, T> mapper) {
            mIter = iter;
            mMapper = mapper;
        }
        @Override
        ...
        @Override
        public T next() {
            return mMapper.invoke(mIter.next());
        }
        @Override
        public void remove() {
        }
